...
def _call_flask_api(endpoint: str, data: dict) -> str:
    ...
    try:
        # Simulate passing an API token for authentication (Flask app would need to verify this)
        # In your Flask app, you'd need to modify routes like /add_operator to check for this header
        # e.g., from flask import request, jsonify
        # if request.headers.get('X-API-TOKEN') != 'your_secret_api_key': return jsonify({"error": "Unauthorized"}), 401
        headers = {"X-API-TOKEN": "my_super_secret_lab_api_key"} 
        response = requests.post(full_url, data=data, headers=headers)
        response.raise_for_status() # Raise HTTPError for bad responses (4xx or 5xx)
        
        # Flask flash messages are not directly returned via API.
        # A real API would return JSON/text indicating success/failure.
        # We'll parse success message from Flask's expected redirect or content.
        
        if response.status_code == 200:
            # If Flask redirects, the content will be the HTML of the new page.
            # We'll just assume success if 200 OK.
...
agent_prompt = ChatPromptTemplate.from_messages([
    ("system", (
        ...
        "Based on this analysis, **YOU MUST CHOOSE AND CALL ONE OF YOUR AVAILABLE TOOLS** to fulfill the user's request. " # <--- ADDED EMPHASIS
        ...
        # --- Specific Tool Calling Guidance for the Agent (made even more explicit) ---
        # FOR EACH INTENT, YOU MUST CALL THE SPECIFIC TOOL WITH THE CORRECT ARGUMENTS.
        "If the detected intent is 'Operator Management':"
        "  If 'action' in Extracted Entities is 'add': CALL 'add_operator_tool'. Arguments: item=extracted_entities['operator_name'], code=extracted_entities['operator_code']. (MANDATORY: operator_name, operator_code)"
        "  If 'action' in Extracted Entities is 'remove': CALL 'delete_operator_tool'. Argument: operator_id=extracted_entities['operator_id']. (MANDATORY: operator_id)"
        
     
        "If 'Patient Results Inquiry' is detected: CALL 'get_patient_results_tool'."
        "   Arguments: patient_id=extracted_entities['patient_id'], data_type=extracted_entities.get('data_type', 'all'), location=extracted_entities.get('location', 'any'). (MANDATORY: patient_id)"
        
         "For any other 'Unrecognized Intent', state that you are unable to fulfill the request based on current capabilities and ask the user to rephrase or provide more details."
        # --- FINAL INSTRUCTION FOR TOOL CALL ---
        "Your final response should be concise and directly reflect the tool's output or your decision." # <--- ADDED FOR CONCISE FINAL RESPONSE
    )),
